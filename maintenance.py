                    ...
                    if not tech or tech.strip() == "":
                        ...
                        for rec in recs:
                            try:
                                ...
                                # ✅ إرسال إشعار Telegram للصيانة الدورية
                                try:
                                    from helpers import send_telegram
                                    msg = f"""✅ <b>صيانة دورية منفذة</b>
⚙️ الماكينة: {rec['machine']}
📏 الخط: {rec['line']}
👨‍🔧 الفني: {rec['technician']}
📋 المهمة: {rec['task']}
🆔 رقم السجل: {record_id}"""
                                    send_telegram(msg)
                                except Exception as tele_e:
                                    print(f"Telegram error (planned): {tele_e}")
        ...
        with st.form("breakdown_form"):
            ...
            issue = st.text_area(t["issue_label"], height=100, key="breakdown_issue_input")

            col1, col2 = st.columns(2)
            with col1:
                start_time = st.time_input(t["start_t"], key="breakdown_start_time")
            ...
            with col2:
                end_time = st.time_input(t["end_t"], key="breakdown_end_time")
            ...
            downtime_category = st.selectbox(
                ...
                t.get("maint_categories", ["ميكانيكي", "كهربائي", "تشغيلي", "مواد خام", "أخرى"]),
            ...
            notes = st.text_area(t["note_label"], key="breakdown_notes")
            ...
            if st.form_submit_button(t["save_btn"], width='stretch'):
                ...
                try:
                    ...
                    st.success(f"{t.get('maint_breakdown_saved', '✅ Breakdown reported successfully')} {record_id})")

                    # ✅ إرسال إشعار Telegram للعطل
                    try:
                        from helpers import send_telegram
                        msg = f"""🔧 <b>عطل جديد</b>
⚙️ الماكينة: {machine}
...
🆔 رقم البلاغ: {record_id}"""
                        ...
                        print(f"Telegram error (breakdown): {tele_e}")
