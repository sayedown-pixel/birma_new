    ...
    # ==================== صيانة أعطال (Breakdown maintenance) ====================
        ...
        with st.form("breakdown_form"):
            ...
            spare_parts = st.text_area(
                ...
                height=80, 
                placeholder="قطع الغيار المستخدمة في الإصلاح...",
            ...
            if submitted:
                ...
                errors = []
                if not tech or tech.strip() == "":
                    errors.append("اسم الفني")
                if not issue or issue.strip() == "":
                    errors.append("وصف العطل")
                
                if errors:
                    st.error(f"⚠️ الرجاء إدخال: {', '.join(errors)}")
                else:
                    ...
                    if end_datetime <= start_datetime:
                        st.error("⚠️ وقت انتهاء الإصلاح يجب أن يكون بعد وقت بدء التوقف")
                    else:
