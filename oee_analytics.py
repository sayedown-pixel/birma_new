...
def show_oee_dashboard(df_main, t, selected_line):
    ...
    with col1:
        days = st.selectbox(
            ...
            [7, 14, 30, 60, 90], 
            index=2,
            key="oee_days_select"  # ✅ مفتاح فريد
        )
    with col2:
        view_type = st.selectbox(
            t.get("oee_view_by", "View by"), 
            ["Daily", "Weekly", "Monthly"],
            key="oee_view_type_select"  # ✅ مفتاح فريد
        )
    try:
    ...
    if not oee_trend.empty and len(oee_trend) > 0:
        ...
        if 'availability' in oee_trend.columns:
            fig_oee.add_trace(go.Scatter(
                ...
                mode='lines',
                name=t.get("oee_availability", "Availability"),
        ...
        if 'performance' in oee_trend.columns:
            fig_oee.add_trace(go.Scatter(
                ...
                mode='lines',
                name=t.get("oee_performance", "Performance"),
        ...
        if 'quality' in oee_trend.columns:
            fig_oee.add_trace(go.Scatter(
                ...
                y=oee_trend['quality'],
                mode='lines',
                name=t.get("oee_quality", "Quality"),
...
def show_downtime_recording_form(t, selected_line):
    ...
    with st.expander(t.get("oee_record_downtime", "📝 Record Downtime Event"), expanded=False):
        with st.form("downtime_form"):
            ...
            if st.form_submit_button(t.get("oee_save_record", "Save Downtime Record"), use_container_width=True):
