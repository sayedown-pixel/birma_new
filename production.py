...
from constants import CONFIG  # ✅ CONFIG من constants.py
from helpers import (
...
def show_production(selected_line, df_raw, df_fg, t):
    ...
    # ✅ تهيئة متغيرات session_state قبل استخدامها
    ...
    # الحصول على اللغة الحالية لاختيار عمود الاسم المناسب
    ...
    with st.expander("📋 " + t.get("raw_stock_expander", "Raw stock"), expanded=False):
        if df_raw is not None and not df_raw.empty:
            ...
            if material_name_col in df_raw.columns:
                ...
                col_label = t.get("material", "Material") if current_lang == 'ar' else "Material"
            else:
                col_label = t.get("material", "Material")
            
            if stock_col and stock_col in df_raw.columns:
                display_cols.append(stock_col)
                stock_label = t.get("current_stock", "Current Stock") if current_lang == 'ar' else "Stock"
            
            if display_cols:
                display_df = df_raw[display_cols].copy()
                rename_dict = {}
                if material_name_col in display_df.columns:
                    rename_dict[material_name_col] = col_label
                if stock_col and stock_col in display_df.columns:
                    rename_dict[stock_col] = stock_label
    ...
    with st.form("prod_form"):
        ...
        with col1:
            supervisor = st.text_input(
                t["sup_label"],
                value="",  # ✅ حقل فارغ
                placeholder=t.get("enter_supervisor_name", "أدخل اسم المشرف"),  # ✅ نص إرشادي
            ...
            st.caption(
                f"📦 {bom['pieces_per_unit']} {t.get('pcs_per_unit', 'pcs/unit')}"
            )
            ...
            # تحديث القيمة في session_state عند تغيير target
        ...
        if submitted:
            if target <= 0:
                ...
                # ✅ استخدام consume_materials_db بدلاً من consume_materials
                raw_ok, raw_msg = consume_materials_db(product, target, selected_line)
                ...
                if not raw_ok:
                    ...
                    try:
                        ...
                        'production',
                        f"Production recorded: {product} - {target} units on {selected_line}",
                        f"Record ID: {record_id}, Efficiency: {metrics['efficiency']}%"
                        )
                        
                        # ✅ استخدام add_to_finished_goods_db بدلاً من add_to_finished_goods
