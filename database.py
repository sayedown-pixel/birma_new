...
# database.py - استبدل دالة check_and_create_alerts بهذه النسخة المحسنة
    ...
    def check_and_create_alerts(self, df_raw=None, df_main=None):
        """فحص وإنشاء التنبيهات تلقائياً - منع التكرار"""
        alerts_created = []
        
        # 1. تنبيهات المخزون المنخفض
        if df_raw is not None and not df_raw.empty:
            for _, row in df_raw.iterrows():
                name_ar = str(row.get('Material_Name_AR', ''))
                current = float(row.get('Current_Stock', 0))
                min_stock = float(row.get('Min_Stock', 0))
                
                if min_stock > 0 and current <= min_stock:
                    # التحقق من وجود تنبيه سابق خلال آخر 24 ساعة
                    existing = self._check_recent_alert('stock', name_ar, hours=24)
                    if not existing:
                        severity = 'critical' if current < min_stock / 2 else 'warning'
                        title = f"⚠️ Low Stock: {name_ar}"
                        message = f"Current stock: {current:,.0f}, Minimum: {min_stock:,.0f}"
                        alert_id = self.add_alert('stock', title, message, severity, None, name_ar)
                        if alert_id:
                            alerts_created.append(alert_id)
        
        # 2. تنبيهات OEE (مرة واحدة فقط في اليوم)
        if df_main is not None and not df_main.empty:
            ...
            if not prod_df.empty and 'oee' in prod_df.columns:
                oee_values = prod_df['oee'].dropna().tail(5)
                if len(oee_values) > 0:
                    latest_oee = oee_values.iloc[-1]
                    
                    if latest_oee < 60:
                        # ✅ التحقق من وجود تنبيه OEE خلال آخر 24 ساعة فقط
                        existing = self._check_recent_alert('oee', 'low_oee', hours=24)
                        if not existing:
                            title = "📉 Low OEE Alert"
                            message = f"Current OEE: {latest_oee:.1f}% (Target: 60%)"
                            alert_id = self.add_alert('oee', title, message, 'critical')
                            if alert_id:
                                alerts_created.append(alert_id)
                                print(f"   ✅ Created OEE alert: {latest_oee:.1f}%")
                        else:
                            print(f"   ⏭️ Skipped OEE alert (already exists in last 24h)")
    ...
    def get_raw_material_by_name(self, name: str, lang: str = 'ar'):
        ...
        try:
            session = self.get_session()
            if lang == 'ar':
                material = session.query(RawMaterial).filter(RawMaterial.name_ar == name, RawMaterial.is_active == True).first()
            else:
                material = session.query(RawMaterial).filter(RawMaterial.name_en == name, RawMaterial.is_active == True).first()
