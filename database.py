...
class DatabaseManager:
    ...
    def get_database_url(self):
        ...
        # طباعة جميع secrets للتأكد
        try:
            logger.info(f"Available secrets keys: {list(st.secrets.keys())}")
        except:
            pass
        
        # قراءة من database section (المستخدمة حالياً)
        try:
            if 'database' in st.secrets:
                db = st.secrets['database']
                
                # إذا كان dict وبه url
                ...
                elif isinstance(db, str) and db.startswith('postgresql://'):
                    logger.info("✅ Found database as string in secrets")
                    return db
                
                # بناء URL من مكونات
                elif isinstance(db, dict) and all(k in db for k in ['host', 'user', 'password', 'database']):
                    port = db.get('port', '5432')
                    url = f"postgresql://{db['user']}:{db['password']}@{db['host']}:{port}/{db['database']}"
                    logger.info("✅ Built database URL from components")
                    return url
                    
        except Exception as e:
            logger.warning(f"Error reading database from secrets: {e}")
        
        # قراءة من postgresql مباشرة
        try:
    ...
    def calculate_oee(self, data: dict) -> dict:
        ...
        planned_time = working if working > 0 else int(data.get('planned_production_time', 450) or 450)
        ...
        actual_output = int(data.get('output_units', 0) or 0)
        
        if operating_time > 0 and ideal_run_rate > 0:
            actual_rate = actual_output / operating_time
            performance = min(100.0, (actual_rate / ideal_run_rate) * 100)
        else:
            performance = 0.0

        waste = max(0, int(data.get('waste_bottles', 0) or 0))
        good_units = max(0, actual_output - waste)
        quality = (good_units / actual_output * 100) if actual_output > 0 else 0
