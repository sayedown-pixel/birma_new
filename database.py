...
def get_database_url():
    ...
    try:
        ...
        # محاولة عدة صيغ ممكنة للـ secrets
        if 'database' in st.secrets:
            ...
            if 'url' in db:
                ...
                return f"postgresql://{db['user']}:{db['password']}@{db['host']}:{port}/{db['database']}"
        
        # محاولة قراءة من postgresql مباشرة
    ...
    def get_stats(self):
        ...
        try:
            session = self.get_session()
             ...
             # إجمالي الإنتاج
            total_production = session.query(Production).count()
            total_units = session.query(Production).with_entities(
                 func.sum(Production.output_units)
            ).scalar() or 0
        
            # إجمالي الصيانة
            total_maintenance = session.query(Maintenance).count()
        
               # إجمالي التسليم
            total_delivery = session.query(Delivery).count()
