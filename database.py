...
def get_database_url():
    ...
    # أولاً: محاولة قراءة من Streamlit Secrets (للسحابة)
    try:
        # الطريقة 1: قراءة من postgresql مباشرة
        if 'postgresql' in st.secrets:
            url = st.secrets['postgresql']
            logger.info("Using PostgreSQL URL from secrets")
        ...
        # الطريقة 2: قراءة من database section
        if 'database' in st.secrets:
            db = st.secrets['database']
            
            # إذا كان هناك URL مباشر
            if 'url' in db:
               url = db['url']
               ...
               logger.info("Using database.url from secrets")
               return url
            
            # بناء URL من مكونات منفصلة
            if all(k in db for k in ['host', 'user', 'password', 'database']):
                port = db.get('port', '5432')
                url = f"postgresql://{db['user']}:{db['password']}@{db['host']}:{port}/{db['database']}"
                logger.info(f"Built database URL from components: {db['host']}")
                return url
                
    except Exception as e:
        logger.warning(f"Could not read from st.secrets: {e}")
    
    # ثانياً: محاولة قراءة من .env (للتشغيل المحلي)
    load_dotenv()
    DATABASE_URL = get_database_url()
    if url:
        logger.info("Using DATABASE_URL from .env")
    else:
        logger.warning("No DATABASE_URL found, will use SQLite")
