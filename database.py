...
class DatabaseManager:
        ...
        try:
            with self.engine.connect() as conn:
                if self._use_sqlite:
                    ...
                    conn.execute(text(f"ALTER TABLE {table} ADD COLUMN IF NOT EXISTS {column} {col_type} DEFAULT {default}"))
                conn.commit()
        except Exception as e:
    ...
    def delete_delivery(self, record_id: int) -> bool:
        """Delete a delivery record (does NOT affect inventory)"""
        session = None
        try:
            session = self.get_session()
    ...
    def delete_raw_receipt(self, record_id: int) -> bool:
        """Delete a raw receipt record (does NOT affect inventory)"""
        session = None
        try:
            session = self.get_session()
...
# ============================================================================
# Delete Records - Public Functions (NEW)
# ============================================================================
