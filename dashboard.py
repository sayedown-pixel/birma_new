                ...
                with col1:
                    ...
                    for rec in critical_items[:5]:
                        ...
                        **{rec['material']}**
                        ...
                        # ✅ إرسال إشعار Telegram للمواد الحرجة
                        try:
                            msg = f"""🔴 <b>تنبيه: مخزون حرج</b>
📦 المادة: {rec['material']}
📊 المتبقي: {rec['current']:,}
🎯 الحد الأدنى: {rec['min_stock']:,}
📦 الكمية المقترحة: {rec['suggested_qty']:,}"""
                            send_telegram(msg)
                        except Exception as e:
                ...
                with col2:
                    ...
                    for rec in warning_items[:5]:
                        ...
                        **{rec['material']}**
                        ...
                        # ✅ إرسال إشعار Telegram للمواد التحذيرية (مرة واحدة فقط)
                        try:
                            msg = f"""🟡 <b>تنبيه: مخزون منخفض</b>
📦 المادة: {rec['material']}
📊 المتبقي: {rec['current']:,}
🎯 الحد الأدنى: {rec['min_stock']:,}
📦 الكمية المقترحة: {rec['suggested_qty']:,}"""
    ...
    # ✅ تنبيهات المخزون المنخفض الإضافية (فحص شامل)
    try:
        if df_raw is not None and not df_raw.empty:
            # تحديد عمود الاسم حسب اللغة
            name_col = 'Material_Name_AR' if lang == 'ar' else 'Material_Name_EN'
            stock_col = 'Current_Stock'
            min_col = 'Min_Stock'
            
            for _, row in df_raw.iterrows():
                material_name = row.get(name_col, '')
                current_stock = row.get(stock_col, 0)
                min_stock = row.get(min_col, 0)
                
                # إذا كان المخزون أقل من أو يساوي الحد الأدنى
                if min_stock > 0 and current_stock <= min_stock:
                    ...
                    # إرسال إشعار Telegram (مع تجنب التكرار)
                    try:
                        import hashlib
                        alert_key = f"stock_alert_{material_name}"
                        if not st.session_state.get(alert_key, False):
                            msg = f"""{urgency} <b>تنبيه المخزون</b>
📦 المادة: {material_name}
📊 المخزون الحالي: {current_stock:,.0f}
🎯 الحد الأدنى: {min_stock:,.0f}
📉 العجز: {max(0, min_stock - current_stock):,.0f}"""
                            send_telegram(msg)
                            st.session_state[alert_key] = True
                    except Exception as e:
                        print(f"Stock alert telegram error: {e}")
    except Exception as e:
