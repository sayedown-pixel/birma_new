        ...
        for material_name, req_qty in required.items():
            ...
            if is_shrink:
                ...
                if material.current_stock < required_rolls:
                    session.rollback()
                    return False, f"⚠️ عجز في المادة {material_name}: المطلوب {required_rolls:.2f} رول، المتوفر {material.current_stock:.2f} رول"
                ...
                if material.current_stock < req_qty:
                    session.rollback()
                    return False, f"⚠️ عجز في {material_name}: المطلوب {req_qty}، المتوفر {material.current_stock}"
                ...
                if material.current_stock < req_qty:
                    session.rollback()
                    return False, f"⚠️ عجز في المادة {material_name}: المطلوب {req_qty:.3f} {material.unit}، المتوفر {material.current_stock:.3f}"
