...
# admin.py - استبدل الاستيراد

from report_generator_reportlab import generate_production_report_pdf, generate_maintenance_report_pdf
...
# admin.py - في دالة _show_reporting_tab، يمكنك إضافة دالة لتنظيف اسم الخط
        ...
        with col2:
            ...
            if _is_prod:
                line_filter = st.selectbox(
                    ...
                    [t.get("admin_all_lines", "All"), t.get("line1", "Line 1"), t.get("line2", "Line 2")],
                    key="line_filter_pdf_main"
                )
        
# admin.py - في دالة _show_reporting_tab، استخدم هذه الدوال فقط

        if st.button(t.get("admin_create_report", "📑 Generate & Download Report"), key="btn_gen_pdf", use_container_width=True):
            with st.spinner(t.get("admin_creating", "Generating report...")):
                ...
                if _is_prod:
                    ...
                    pdf_path = generate_production_report_pdf(start_date, end_date, line_param)  # ✅ لا تمرر lang
                else:
                    pdf_path = generate_maintenance_report_pdf(start_date, end_date)  # ✅ لا تمرر lang
        ...
        with st.form(key="email_report_form"):
            ...
            with col1:
                recipient = st.text_input(
                    ...
                    help="Enter the email address where you want to receive the report",
            ...
            with col2:
                # اختيار خط الإنتاج (لتقارير الإنتاج فقط)
    ...
    with report_tab3:
        ...
        if st.button(t.get("admin_show_comparison", "📊 Show Comparison"), key="btn_compare", use_container_width=True):
...
def show_settings(t):
    ...
    with tab_password:
        ...
        with st.form("change_pw_form"):
            ...
            if st.form_submit_button(t.get("admin_change_pw_btn", "Change Password"), use_container_width=True):
