# admin.py - نسخة نظيفة بدون clean_line_name
...
def _show_reporting_tab(t):
    ...
    with report_tab2:
        ...
        # ✅ زر إرسال لجميع المستلمين (خارج الفورم)
        col_btn1, col_btn2 = st.columns(2)
        with col_btn1:
            if st.button("📧 إرسال التقرير لجميع المستلمين (فوري)", key="send_to_all", width='stretch'):
                with st.spinner("جاري إرسال التقارير لجميع المستلمين..."):
                    from email_sender import send_weekly_auto_reports
                    send_weekly_auto_reports()
                    st.success("✅ تم إرسال التقارير لجميع المستلمين")
        with col_btn2:
            st.info("📅 **الإرسال التلقائي:** كل يوم اثنين الساعة 8 صباحاً")
        
        st.markdown("---")
        
        # ✅ نموذج الإرسال لبريد واحد
