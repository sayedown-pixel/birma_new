...
def get_auto_reorder_suggestions(df_raw, df_main):
    ...
    for _, row in df_raw.iterrows():
        ...
        # ✅ تشمل جميع المواد المخزنة (حتى التي ليس لها حد أدنى)
        # إذا كان الحد الأدنى = 0، نستخدم قيمة افتراضية (مثلاً 7 أيام استهلاك)
        if min_stock == 0:
            # يمكننا استخدام daily_consumption إذا كان متاحاً
            daily_consumption = float(row.get('Daily_Consumption', 0)) if pd.notna(row.get('Daily_Consumption', 0)) else 0
            if daily_consumption > 0:
                min_stock = daily_consumption * 7  # 7 أيام كحد أدنى افتراضي
        
        # ✅ تشمل جميع المواد (وليس فقط التي وصلت للحد الأدنى)
        if min_stock > 0:
            if current <= min_stock:
                ...
                suggestions.append({
                    ...
                    "urgency": urgency,
                    "level": level,
                    "percentage": (current / min_stock * 100) if min_stock > 0 else 0
                })

    # ترتيب حسب الأكثر خطورة أولاً
    suggestions.sort(key=lambda x: (x['urgency'] == 'critical', x['urgency'] == 'warning'), reverse=True)
    return suggestions
