...
def calculate_production_metrics(
    ...
    """
    units               = كمية الإنتاج بالوحدة (كرتون / شرنك).
    speed_bottles_per_hour = سرعة الخط بالبريفورم (عبوة) في الساعة.
    preforms_used       = إجمالي البريفورم المستهلك فعلياً (عبوة).
    packaging_used      = وحدات التغليف المستهلكة فعلياً (كرتون / شرنك).

    منطق الحسابات:
    ─────────────────────────────────────────────────────────────
    good_bottles    = units × pieces_per_unit   ← العبوات الجيدة
    final_preforms  = preforms_used (أو good_bottles إن لم يُدخل)
    waste_bottles   = final_preforms - good_bottles   ← هالك العبوات

    الكفاءة تُقاس بالبريفورم المُدخل الفعلي نسبةً لسرعة الخط:
        efficiency = (final_preforms / theoretical_bottles) × 100

    التوقف يُحسب من البريفورم المُدخل:
        required_hours = final_preforms / speed_bottles_per_hour
        downtime       = working_hours - required_hours

    هالك التغليف:
        expected_packaging = units (كرتون/شرنك)
        packaging_waste    = packaging_used - units   (إن > 0)
    ─────────────────────────────────────────────────────────────
    """
    ...
    # ── العبوات الجيدة المنتجة ─────────────────────────────────
    good_bottles = units * pieces_per_unit

    # ── البريفورم الفعلي المستخدم ──────────────────────────────
    # إذا أدخل المستخدم قيمة نستخدمها، وإلا نفترض لا هالك
    final_preforms = int(preforms_used) if int(preforms_used) > 0 else good_bottles

    # ── هالك العبوات = فرق البريفورم عن الإنتاج الجيد ─────────
    ...
    try:
        ...
        # ── هالك التغليف (بالوحدة: كرتون / شرنك) ───────────────
        expected_packaging = units           # المتوقع = عدد الوحدات المنتجة
        ...
        return {
            ...
            "bottles_produced":     good_bottles,   # للتوافق مع الكود القديم
