...
LANG = {
    "ar": {
        ...
        "line1": "الخط الأول (smi)",
        "line2": "الخط الثاني (welbing)",
    ...
    "en": {
        ...
        "role": "Role",
        "dark_mode": "Dark Mode",
        "app_title": "BIRMA Integrated System",
        "dashboard": "🏠 Dashboard",
...
def consume_materials(product, quantity, df_raw):
    ...
    for norm_material, req in required.items():
        ...
        if current < req:
            shortages.append(f"{norm_material} (مطلوب {req:,.0f} ، متوفر {current:,.0f})")
        else:
...
def get_shift_info():
    ...
    break_times = [
        {"start": now.replace(hour=10, minute=0, second=0), "end": now.replace(hour=10, minute=30, second=0), "duration": 0.5},
        {"start": now.replace(hour=13, minute=0, second=0), "end": now.replace(hour=14, minute=0, second=0), "duration": 1.0},
        {"start": now.replace(hour=18, minute=0, second=0), "end": now.replace(hour=18, minute=30, second=0), "duration": 0.5},
        {"start": now.replace(hour=23, minute=0, second=0), "end": next_day.replace(hour=0, minute=30, second=0), "duration": 1.0},
    ]
    ...
    return {
        ...
        "total_break_hours": 3,
        "working_hours": shift_duration_hours - 3,
...
def create_machine_file(filepath):
    if "Compressor" in filepath or "AF_Compressor" in filepath:
        sample_data = pd.DataFrame({
            "Cat": ["Daily checks","Operation check","Cleaning & Lubrication","Cleaning & Lubrication","Mechanical check","Mechanical check","Auxiliary systems","Auxiliary systems","Electrical","Advanced maintenance","Advanced maintenance","Advanced maintenance"],
            "No": [1,1,1,2,1,2,1,2,1,1,2,3],
            "Name": ["Fill weekly performance log","Check emergency stop button","Check oil level","Replace or clean oil filter","Check belt tension and alignment","Clean intake air filter","Check Bekomats drain operation","Clean dryer cooler","Electrical cabinet filter","Valve cover bolt torque","Motor bearing lubrication","Clean main motor"],
            "Photo": [""]*12,
            "Tools": ["Pen","Manual","Oil gauge","Filter wrench","Tension gauge","Compressed air","Visual inspection","Soft brush","Vacuum/air","Torque wrench","Grease gun","Compressed air"],
            "Proc": ["Record data","Manual test","Check correct marking","Clean or replace","Check tension and alignment","Clean from dust","Check for no blockage","Clean fins","Clean for ventilation","Per specifications","Sufficient grease","Free from contaminants"],
            "Freq": ["Daily","Daily","Daily","Weekly","Weekly","Weekly","Weekly","Weekly","Weekly","Monthly","Yearly","Yearly"],
        ...
        sample = pd.DataFrame({
            ...
            "No": [1,2,3,4,5],
            "Name": ["Check bearings","Clean filters","Calibrate sensors","Lubricate parts","Check belts"],
            "Photo": [""]*5,
            "Tools": ["Wrench","Brush + air","Calibration device","Grease","Wrench"],
            "Proc": ["Check vibrations","Clean with air","Calibrate per manual","Lubricate every 100 hours","Check tension"],
...
def get_marquee_recommendations(df_raw, df_main, df_fg, t, lang, selected_line):
    ...
    if df_fg is not None and not df_fg.empty and "Balance" in df_fg.columns:
        ...
        if fg_balance <= 0:
            ...
            recommendations.append(f"📦 {t.get('fg_balance', '')}: {fg_balance:,.0f} {t.get('marquee_fg_units', 'units' if en else 'وحدة')}")
...
# ============================================================================
# Delete Production Record (لازم يكون في آخر الملف عشان يتجنب الاستيراد الدائري)
# ============================================================================

def delete_production_record(record_id, df_raw, df_fg):
    from database import db_manager
    from inventory import update_raw_materials, update_finished_goods, restore_finished_goods_from_production
