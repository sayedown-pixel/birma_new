...
def create_database_backup():
    ...
    try:
        ...
        update_backup_log()
        return True, f"✅ تم إنشاء النسخة الاحتياطية {timestamp}"
        
    except Exception as e:
        return False, f"❌ فشل إنشاء النسخة الاحتياطية: {str(e)}"

def run_auto_backup():
    """تشغيل النسخ الاحتياطي التلقائي"""
    if should_create_backup():
        return create_database_backup()
    return True, "✅ لا حاجة لنسخة احتياطية جديدة (تم اليوم)"
...
def restore_from_backup(backup_info):
    ...
    try:
        ...
        return True, f"✅ تم استعادة {len(restored_tables)} جدول: {', '.join(restored_tables)}"
...
def create_manual_backup(description=""):
    ...
    try:
        ...
        return True, f"✅ تم إنشاء النسخة الاحتياطية اليدوية {timestamp}"
...
def show_backup_management(t):
    ...
    with tab_backup:
        ...
        with col1:
            if st.button(t.get("backup_auto_btn", "🤖 Run Auto Backup"), use_container_width=True):
        ...
        with col2:
            ...
            if st.button(t.get("backup_manual_btn", "📀 Create Manual Backup"), use_container_width=True):
                with st.spinner(t.get("backup_creating", "Creating backup...")):
    ...
    with tab_restore:
        ...
        if backups:
            ...
            with col2:
                if confirm == "RESTORE":
                    if st.button(t.get("backup_restore_btn", "🔄 Restore Now"), type="primary", use_container_width=True):
    ...
    with tab_export:
        ...
        with col1:
            ...
            if st.button(t.get("backup_export_excel_btn", "📑 Export All to Excel"), use_container_width=True):
        ...
        with col2:
            ...
            if st.button(t.get("backup_export_pdf_btn", "📄 Generate PDF Report"), use_container_width=True):
