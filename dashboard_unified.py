...
def show_smart_recommendations(df_raw, df_main, selected_line, t):
    """عرض التوصيات الذكية - عرض جميع المواد المنخفضة"""
    lang = st.session_state.get('lang', 'ar')
    st.markdown("---")
    st.subheader("📊 " + t.get("smart_recommendations", "Smart Recommendations"))
    
    # ==================== توصيات إعادة الطلب - عرض الكل ====================
    ...
    # عرض المواد الحرجة (كلها)
    if critical_items:
        critical_title = t.get("critical_reorder", "🔴 Critical - Need Immediate Action")
        st.markdown(f"##### {critical_title}")
        
        for rec in critical_items:
            if rec.get('current', 0) <= 0:
                st.error(f"""
                🚨 **{rec['material']}** - **منفذ بالكامل!**
                ...
                - {t.get('min_label', 'Min')}: {rec['min_stock']:,}
                - {t.get('suggested_reorder', 'Suggested')}: {rec['suggested_qty']:,}
                - {t.get('percentage', 'Percentage')}: {rec.get('percentage', 0)}%
                """)
    else:
        st.info(f"✅ {t.get('no_critical', 'No critical materials')}")
    
    # عرض المواد التحذيرية (كلها)
    if warning_items:
        warning_title = t.get("warning_reorder", "🟡 Warning - Plan Reorder Soon")
    ...
    # ✅ عرض العدد الإجمالي للمواد المنخفضة
    total_low = len(critical_items) + len(warning_items)
    if total_low > 0:
        st.caption(f"📊 إجمالي المواد المنخفضة: {total_low} مادة ({len(critical_items)} حرجة, {len(warning_items)} تنبيه)")
        ...
        if critical_pred:
            for pred in critical_pred:
                if pred.get('days_left', 0) <= 0:
                    st.error(f"🚨 **{pred['material']}** - منفذ بالكامل!")
                else:
...
def show_dashboard(df_main, df_raw, df_fg, t, selected_line):
    ...
    # العنوان حسب اللغة
    if lang == 'ar':
        title_text = "🏭 نظام المصنع الذكي"
        subtitle_text = "لوحة القيادة الرئيسية"
    else:
        title_text = "🏭 Smart Factory System"
        subtitle_text = "Main Dashboard"
