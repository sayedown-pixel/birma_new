...
import pandas as pd
import numpy as np  # ✅ تأكد من وجود هذا السطر
...
class PredictiveAnalytics:
    ...
    def get_oee_improvement_suggestions(self):
        ...
        waste_percentage = (total_waste / (total_units * 20)) * 100
...
def show_predictive_analytics(t, selected_line):
    ...
    with tab2:
        ...
        if df_production is not None and not df_production.empty and 'oee' in df_production.columns:
            ...
            # ✅ التحقق من وجود بيانات كافية لخط الاتجاه
            trend_line = None
            if len(daily_oee) >= 3:
                x = np.arange(len(daily_oee))
                y = daily_oee['oee'].values
                
                # التحقق من أن القيم ليست كلها متطابقة
                if len(np.unique(y)) > 1:
                    try:
                        ...
                        # إذا فشل polyfit، استخدم خط أفقي
                        trend_line = np.full_like(x, y.mean())
                else:
                    # كل القيم متطابقة - استخدم خط أفقي
                    trend_line = np.full_like(x, y.mean())
            
            # إضافة خط الاتجاه إذا وجد
            ...
            fig.update_layout(
                title=t.get("oee_daily_trend", "Daily OEE Trend"),
            ...
            # توقع OEE للأيام القادمة (فقط إذا كانت البيانات كافية)
            if len(daily_oee) >= 5:
                last_7_avg = daily_oee.tail(7)['oee'].mean()
                
                # حساب الاتجاه
                if len(daily_oee) >= 3 and len(np.unique(daily_oee['oee'].values)) > 1:
                    x = np.arange(len(daily_oee))
                    y = daily_oee['oee'].values
                    z = np.polyfit(x, y, 1)
                    trend_slope = z[0]
                    next_oee = last_7_avg + trend_slope * 7
                    trend_text = '📈 Improving' if trend_slope > 0 else '📉 Declining' if trend_slope < 0 else '➡️ Stable'
                else:
                    trend_slope = 0
                    next_oee = last_7_avg
                    trend_text = '➡️ Stable (insufficient data)'
