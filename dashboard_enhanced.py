...
def show_kpi_cards(df_main, t):
    ...
    # عرض البطاقات في صفوف
    col1, col2, col3, col4 = st.columns(4)
    
    with col1:
        # لون المؤشر حسب الأداء
...
# dashboard_enhanced.py - استبدل دالة show_line_comparison بهذه النسخة
...
def show_line_comparison(df_main, t):
    ...
    # ✅ استخدام أسماء الأعمدة الفعلية بدلاً من الترجمة المباشرة
    # أسماء الأعمدة في line_stats هي: 'output_units', 'efficiency', 'oee', 'downtime_minutes'
    ...
    for i, line in enumerate(lines):
        with cols[i]:
            ...
            total_prod = stats['output_units']
            avg_eff = stats['efficiency']
            avg_oee_val = stats['oee']
            total_dt = stats['downtime_minutes']
            
            st.metric(f"🏭 {line}", f"{total_prod:,.0f}")
            st.caption(f"⚡ {t.get('efficiency', 'Efficiency')}: {avg_eff:.1f}%")
            st.caption(f"📈 {t.get('avg_oee', 'OEE')}: {avg_oee_val:.1f}%")
            st.caption(f"⏰ {t.get('downtime', 'Downtime')}: {total_dt:.0f} {t.get('minutes_word', 'min')}")


def show_line_comparison_table(df_main, t):
    """عرض جدول مقارنة خطوط الإنتاج"""
    ...
    display_stats = line_stats.copy()
    display_stats.columns = [
        t.get("total_production", "Total Production"),
        t.get("avg_efficiency", "Avg Efficiency %"),
        t.get("avg_oee", "Avg OEE %"),
        t.get("total_downtime", "Total Downtime (min)")
    ]
