...
def get_materials_required(product, quantity, preforms_used=0, packaging_used=0):
    ...
    """
    import math
    from constants import BOM, SHRINK_ROLL_CONVERSION
    ...
    for material, qty in BOM[product].items():
        if "بريفورم" in material:
            ...
            # ✅ حساب الشرنك بالقطع أولاً (سيتم تحويله إلى رول في consume_materials_db)
            if packaging_used > 0:
                req_qty = qty * packaging_used
                print(f"   📦 {material}: {qty} rolls/unit × {packaging_used} units = {req_qty} rolls required")
            else:
                req_qty = qty * quantity
                print(f"   📦 {material}: {qty} rolls/unit × {quantity} units = {req_qty} rolls required")
            
            # تحويل الرول إلى قطع (سيتم استخدامه في المقارنة مع المخزون بعد التحويل)
            pieces_per_roll = SHRINK_ROLL_CONVERSION.get(material, 1980)
            req_qty_pieces = req_qty * pieces_per_roll
            print(f"   📦 {material}: {req_qty} rolls × {pieces_per_roll} pieces/roll = {req_qty_pieces} pieces")
            
            # نخزن الكمية بالقطع للاستخدام في الدوال الأخرى
            req_qty = req_qty_pieces
