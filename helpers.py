...
def get_shift_info():
    """الحصول على معلومات الوردية الحالية - مع دعم اللغة"""
    from datetime import datetime, timedelta
    ...
    # الحصول على نص الوردية من الترجمة
    shift_name = LANG.get(current_lang, {}).get("shift_single", "الوردية الواحدة (8 صباحاً - 2 صباحاً)")
    
    return {
        "shift_start": now.replace(hour=8, minute=0, second=0),
        "shift_end": next_day.replace(hour=2, minute=0, second=0),
