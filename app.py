...
def show_sidebar(t):
    ...
    with st.sidebar:
        ...
        with col_l:
            ...
            if current_lang == 'ar':
                if st.button("🇬🇧 " + t.get("english_btn", "English"), use_container_width=True):
                    st.session_state.lang = 'en'
                    st.session_state.t = load_language('en')
                    st.rerun()
            else:
                if st.button("🇸🇦 " + t.get("arabic_btn", "عربي"), use_container_width=True):
        ...
        # ✅ قائمة الصفحات - تعرف هنا داخل الدالة حيث t معرف
        page_mapping = {
